...
/** Helpers globaux utilisés par les vues admin. */
...
/** Message flash one-shot (succès / erreur) stocké en session. */
...
function format_datetime(?string $value): string
{
    ...
    $ts = strtotime($value);
    return $ts ? date('d/m/Y H\hi', $ts) : $value;
}

function format_price(null|int|float|string $amount, string $currency = 'F CFA'): string
{
...
/**
 * Pagination compacte (chevrons + numéros fenêtrés + état actif), même rendu
 * que la liste des inscriptions. $query : filtres à conserver (sans 'page').
 */
function pagination(int $page, int $pages, string $baseUrl, array $query = []): string
{
    if ($pages <= 1) {
        return '';
    }
    $query = array_filter($query, static fn ($v) => $v !== null && $v !== '');
    $url = static fn (int $p): string => e($baseUrl . '?' . http_build_query(array_merge($query, ['page' => $p])));
    ...
    $html .= '<a class="page-btn ' . ($page <= 1 ? 'page-btn-disabled' : '') . '" href="' . $url(max(1, $page - 1))
        . '" title="Page précédente">' . icon('chevron-left', 'h-4 w-4') . '</a>';
    $prev = 0;
    foreach ($shown as $p) {
        if ($p - $prev > 1) {
        ...
        $html .= $p === $page
            ...
            : '<a class="page-btn" href="' . $url($p) . '">' . $p . '</a>';
        $prev = $p;
    }
    $html .= '<a class="page-btn ' . ($page >= $pages ? 'page-btn-disabled' : '') . '" href="' . $url(min($pages, $page + 1))
        . '" title="Page suivante">' . icon('chevron-right', 'h-4 w-4') . '</a>';
    return $html . '</nav>';
}

/**
 * Icône SVG inline (style Heroicons outline) — compatible CSP (pas de font externe).
 * Usage : <?= icon('users', 'h-5 w-5') ?>
 */
function icon(string $name, string $class = 'h-5 w-5'): string
{
    static $icons = [
        ...
        'user-plus' => '<path stroke-linecap="round" stroke-linejoin="round" d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM3 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 019.374 21c-2.331 0-4.512-.645-6.374-1.766z"/>',
        ...
        'credit-card' => '<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5a2.25 2.25 0 002.25 2.25z"/>',
        ...
        'identification' => '<path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5a2.25 2.25 0 002.25 2.25zm4.125-9.75a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z"/>',
        ...
        'chat-bubble' => '<path stroke-linecap="round" stroke-linejoin="round" d="M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 01.778-.332 48.294 48.294 0 005.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"/>',
    ];
    $path = $icons[$name] ?? $icons['information-circle'];
    return '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8"'
        . ' stroke="currentColor" class="' . e($class) . '" aria-hidden="true">' . $path . '</svg>';
}
